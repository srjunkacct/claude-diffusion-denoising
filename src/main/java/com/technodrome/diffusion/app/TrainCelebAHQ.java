...
import com.technodrome.diffusion.dataset.CelebAHQDataset;
...
/**
 * CelebA-HQ 256x256 unconditional diffusion training.
 * Ported from run_celebahq.py.
 *
 ...
 */
@Command(name = "train-celebahq", description = "Train DDPM on CelebA-HQ 256x256")
public class TrainCelebAHQ implements Runnable {

    private static final Logger logger = LoggerFactory.getLogger(TrainCelebAHQ.class);
    ...
    @Option(names = "--data-dir", required = true, description = "Directory containing CelebA-HQ images")
    Path dataDir;

    @Option(names = "--output-dir", defaultValue = "output/celebahq", description = "Output directory")
    Path outputDir;

    @Option(names = "--seed", defaultValue = "42", description = "Random seed")
    long seed;

    @Option(names = "--randflip", defaultValue = "true", description = "Random horizontal flip")
    boolean randflip;
    ...
    public void run() {
        try {
            ...
            logger.info("Training CelebA-HQ 256x256 DDPM");
            logger.info("Config: lr={}, bs={}, dropout={}, timesteps={}, loss={}",
                    lr, batchSize, dropout, timesteps, lossType);
            ...
            // Dataset
            Dataset dataset = CelebAHQDataset.getTrainDataset(dataDir, batchSize, 256, true);
            dataset.prepare();
    ...
    public static void main(String[] args) {
        int exitCode = new CommandLine(new TrainCelebAHQ()).execute(args);
